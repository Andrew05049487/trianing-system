...
import com.example.trainingsystems.service.ChatApiException;
...
public class ChatExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleApiError(ChatApiException error) {
    ...
    public ResponseEntity<ApiErrorResponse> handleMalformedRequest(Exception error) {
        ...
        return ResponseEntity.status(status).body(
            ApiErrorResponse.of(
                status.value(),
                status.getReasonPhrase(),
                "Request header、path 或 JSON 格式錯誤"
            )
        );
    }
